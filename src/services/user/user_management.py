...
from src.services.auth.jwt_claims import extract_user_data_from_jwt
...
class UserManagementService(BaseService):
    ...
    async def remove_user_from_organization(self, user_id: UUID) -> bool:
        ...
        if not user_org:
            # Create user organization if it doesn't exist
            from src.services.user_onboarding_service import UserOnboardingService
            ...
            _, user_org = await onboarding_service.ensure_user_onboarded(
                ...
                email=user.email,
                name=user.name,
                plan_tier=PlanTier.FREE,  # Default plan tier for user organization
            )
        ...
        # Invalidate the specific onboarding cache for this user
    ...
    @cached(10800)  # 3 hours TTL - onboarding is essentially a one-time event
    async def ensure_user_onboarded_cached(
        ...
        Uses Redis cache to avoid repeated onboarding checks for the same user.
        Cache key automatically generated by @cached decorator
        Cache TTL: 3 hours - onboarding is essentially a one-time event
        """
        logger.debug(f"Onboarding check for user: {user_id}")

        # Perform onboarding - decorator handles caching
        onboarding_service = UserOnboardingService(self.db)
    ...
    async def validate_organization_access(
        ...
        Returns the organization if valid, raises exception if not.
        """
    ...
    async def handle_jwt_authentication(
        ...
        """
        Handle JWT authentication with user onboarding and org validation.

        Returns (user, organization) if successful.
        """
        ...
        # Ensure user is onboarded with user organization
        ...
        # Return user's user organization (no header-based organization validation needed)
        return user, user_org
