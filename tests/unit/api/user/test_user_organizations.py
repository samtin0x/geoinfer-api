...
async def test_set_active_organization_success(
    ...
    # Verify other organizations are not active
    for org in updated_orgs:
        if org["id"] != str(org2.id):
            assert org["is_active"] is False, "Only one organization should be active"
...
async def test_organization_switch_updates_context(
    ...
    for target_org in orgs:
        response = await authorized_client.patch(
            f"/v1/user/organizations/{target_org.id}/active"
        )
