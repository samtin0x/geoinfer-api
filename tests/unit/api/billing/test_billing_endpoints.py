...
async def test_billing_endpoints_require_billing_permission(
    app,
    authorized_client_no_billing: AsyncClient,
    test_user_no_billing_perms,
    db_session,
):
    """Test that billing endpoints require billing management permission."""
    fake_subscription_id = str(uuid4())
    ...
    for endpoint in endpoints_to_test:
        response = await authorized_client_no_billing.get(endpoint)
        assert (
            response.status_code == status.HTTP_403_FORBIDDEN
        ), f"Endpoint {endpoint} should require billing permission"


@pytest.mark.asyncio
async def test_alert_endpoints_require_subscribed_plan(
    app, authorized_client_free_tier: AsyncClient, test_user_free_tier, db_session
):
    """Test that alert endpoints require SUBSCRIBED plan tier."""
    fake_subscription_id = str(uuid4())
    ...
    for endpoint in alert_endpoints:
        if endpoint.endswith("/test-alert"):
            response = await authorized_client_free_tier.post(endpoint)
        else:
            response = await authorized_client_free_tier.get(endpoint)

        assert (
            response.status_code == status.HTTP_403_FORBIDDEN
        ), f"Endpoint {endpoint} should require SUBSCRIBED plan"


@pytest.mark.asyncio
async def test_alert_endpoints_allow_subscribed_plan(
    app,
    authorized_client_subscribed: AsyncClient,
    test_user_subscribed,
    db_session,
    test_subscription_subscribed,
):
    """Test that alert endpoints allow SUBSCRIBED plan tier."""
    subscription_id = str(test_subscription_subscribed.id)

    # Test GET alert settings
    response = await authorized_client_subscribed.get(
        f"/v1/billing/subscriptions/{subscription_id}/alert-settings"
    )
    assert response.status_code == status.HTTP_200_OK

    # Test checking usage alerts
    response = await authorized_client_subscribed.get("/v1/billing/alerts/usage")
    assert response.status_code == status.HTTP_200_OK

    # Test test alert (should be rate limited but not forbidden)
    response = await authorized_client_subscribed.post(
        f"/v1/billing/subscriptions/{subscription_id}/test-alert"
    )
    # Should either succeed (200) or be rate limited (429), but not forbidden (403)
